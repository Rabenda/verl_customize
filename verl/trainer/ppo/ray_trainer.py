...
class RayPPOTrainer:
    ...
    def init_workers(self):
        ...
        # logic: create actor rollout worker group
        # debug 
        print(f"[debug], rollout {all_wg}")
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        # 记录 Rollout 轨迹点（已关闭 profiler 打点，避免刷屏）
                    ...
                    # 方案 B: 每个 step 将本 batch 的 decoding length 按样本顺序追加到 CSV（single/multi turn 通用）
                    # multi-turn 时若 batch 带 __num_turns__，则多写一列 turn，便于按 turn 区分
                    _decoding_length_csv_dir = self.config.trainer.get("decoding_length_csv_dir", None)
                    if _decoding_length_csv_dir is not None:
                        os.makedirs(_decoding_length_csv_dir, exist_ok=True)
                        ...
                        csv_path = os.path.join(_decoding_length_csv_dir, f"lengths_{safe_name}.csv")
                    ...
                    # --- 2. Update 阶段 (生成图中的矩形方块) ---
                    if self.use_critic:
                        # 记录训练开始（profiler 打点已关闭）
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        # 记录 Actor 训练开始点（profiler 打点已关闭）
                        # train_block_start = time.time() - fit_start_time
                        # logger.log({"profiler/logical_utilization_pct": 100.0}, step=int(train_block_start * 1000))
                        with marked_timer("update_actor", timing_raw, color="red"):
                            actor_output = self._update_actor(batch)
                        # 记录 Actor 训练结束点（profiler 打点已关闭）
                        # train_block_end = time.time() - fit_start_time
                        # logger.log({"profiler/logical_utilization_pct": 0.0}, step=int(train_block_end * 1000))
