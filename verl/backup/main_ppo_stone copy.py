...
def _make_actor_cfg_from_shared(config, which: str):
    assert which in ("a", "b")
    shared = config.actor_rollout_ref
    path_key = f"path_{which}"
    n_key = f"n_{which}"

    if not hasattr(shared.model, path_key):
        raise ValueError(f"Missing actor_rollout_ref.model.{path_key}")
    if not hasattr(shared.rollout, n_key):
        raise ValueError(f"Missing actor_rollout_ref.rollout.{n_key}")

    scoped = deepcopy(shared)
    with open_dict(scoped):
        scoped.model.path = getattr(shared.model, path_key)
        scoped.rollout.n = getattr(shared.rollout, n_key)
    return scoped
...
class TaskRunner:
    ...
    def add_actor_rollout_workers(self, config):
        ...
        # We do not support lora for now
        assert config.actor_rollout_ref.model.get("lora", {}).get("rank", 0) <= 0
    ...
    def run(self, config):
        ...
        """
        # Check if dual model training is enabled in the configuration
    ...
    def run_dual_model(self, config):
        ...
        # add two actor rollout workers
        actor_rollout_cls, ray_worker_group_cls = self.add_actor_rollout_workers(config)

        # critic / reward / ref , same as single model
        # we now assume the dual model shares the same backbone for critic / reward / ref
        ...
        # still assuming tokenizer / processor are shared
        local_path = copy_to_local(
            config.actor_rollout_ref_a.model.path, use_shm=config.actor_rollout_ref_a.model.get("use_shm", False),
        )
