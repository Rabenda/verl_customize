...
# Set to True to log each inference step (mode, batch_size, prefill/decode tokens)
LOG_INFERENCE_STEP = True
# Base name for inference step CSV (suffix from SGLANG_INFERENCE_LOG_SUFFIX env)
LOG_INFERENCE_STEP_CSV_BASE = "inference_step_log"
...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def _select_green_stream(self, forward_batch: ForwardBatch):
        ...
        if self.green_streams is None:
            return None
        algo = os.environ.get("SGLANG_GREENCTX_ALGO", "disable")
        if algo == "disable":
            return None
        if algo == "naive":
    ...
    def forward(
        ...
        stream = self._select_green_stream(forward_batch)

        if stream is not None:
            stream.wait_stream(self.forward_stream)

        stream_ctx = (
            torch.get_device_module(self.device).stream(stream)
            if stream is not None
            else contextlib.nullcontext()
        )
        # if self.forward_pass_id % 20 == 0:
        #     print(f"[{self.forward_pass_id}] stream={stream}")
        with stream_ctx:
        ...
        # Ensure forward_stream waits for green_stream before sample/copy_to_cpu
        if stream is not None:
            self.forward_stream.wait_stream(stream)
        ...
        if LOG_INFERENCE_STEP and self.tp_rank == 0:
            if self.device in ("cuda", "npu"):
                torch.get_device_module(self.device).synchronize()
            forward_time_ms = (time.perf_counter() - _log_t0) * 1000
            prefill_tokens = forward_batch.extend_num_tokens or 0
            decode_tokens = (
                forward_batch.batch_size if forward_batch.forward_mode.is_decode() else 0
            )
            avg_seq_len = (
                forward_batch.seq_lens_sum / forward_batch.batch_size
                if forward_batch.batch_size > 0
                else 0.0
            )
