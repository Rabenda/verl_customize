...
# 复用单曲线脚本里的硬件/模型配置与 MFU 计算逻辑，保证与其它图完全一致
...
# 1) gsm8k | Qwen3-4B
# 2) math  | Qwen3-4B
# 3) aime  | Qwen3-4B-instruct
# 4) aime  | Qwen3-32B
FILES: List[Tuple[str, str, str]] = [
    ("inference_step_log_multiturn_gsm8k_qwen3-4b.csv", "Qwen3-4B", "GSM8K"),
    ("inference_step_log_multiturn_math_qwen3-4b.csv", "Qwen3-4B", "MATH"),
    ("inference_step_log_multiturn_aime-qwen3-4b-instruct.csv", "Qwen3-4B-instruct", "AIME"),
    ("inference_step_log_multiturn_aime_qwen3-32b.csv", "Qwen3-32B", "AIME"),
]


def _parse_first_step_metrics(csv_path: str, min_prefill_tokens: int = 5000) -> List[Dict]:
    """解析 multiturn inference_step_log 第一个 global step 的 metric 列表。

    与 draw_mfu_plot 中 CSV 分支保持一致：
    - 如果有 global_step 列：只取 global_step == "1"
    - 否则：用 find_global_step_boundaries 取第一个 step
    - 忽略 pass_id == 1 的行
    """
    metrics: List[Dict] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="四个 multiturn 实验的 MFU 1x4 子图 (zhengding 版)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="输出 PDF 路径（默认 pic/new2.pdf）",
    )
    ...
    out_path = args.save or os.path.join(pic_dir, "new2.pdf")
