...
CSV 与本脚本同目录：lengths_model_multiturn_*.csv（列 length,turn）。
First step：取前 chunk_size 行（与 decoding length first-step 一致）。
绘图风格：绿/蓝/橙/紫。默认输出矢量 PDF（可用 --save 指定路径）。
"""
...
# 数据与脚本同目录，便于单独打包 turn_plot 离线出图
DATA_DIR = DIR
...
def main():
    parser = argparse.ArgumentParser(description="Turn vs frequency 四配置柱状图（矢量 PDF）")
    parser.add_argument(
        "--save",
        type=str,
        default=os.path.join(DIR, "pic", "turn_frequency_four_multiturn.pdf"),
        help="输出路径（.pdf 推荐；亦支持 .png 等 matplotlib 支持的格式）",
    )
    ...
    # 断轴双 subplot 与 tight_layout 易冲突，用手动边距 + savefig bbox_inches
    fig.subplots_adjust(left=0.12, right=0.98, top=0.92, bottom=0.12, hspace=0.08)
